...
final case class PublicTransportCombinationsRequest(lat: Double, lng: Double, radius: Option[Double], line: Option[String])
...
trait Protocol extends DefaultJsonProtocol with CaseObjectSerializationSupport with ModelFormatter with RouteFormatter {
  ...
  implicit val PublicTransportCombinationsRequestFormat = jsonFormat4(PublicTransportCombinationsRequest.apply)
