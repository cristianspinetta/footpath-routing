...
object DBTest extends App with LazyLoggerSupport with MeterSupport {
  ...
  //  newNode(GeoNode(Coordinate(2, 5)))
...
//  val testCasesFut = Future.sequence(testCases.flatMap(f => executeTestCase(f)))
  ...
  def generateData(): Unit = {
    val (rows: Int, columns: Int) = (1, 1)

    val dataSet: Vector[Coordinate] = (for (_ <- 1 to 1000000) yield randomCoordinate(r)).toVector

    val nodes = withTimeLogging(saveCoordinate(dataSet), { timing: Long ⇒
