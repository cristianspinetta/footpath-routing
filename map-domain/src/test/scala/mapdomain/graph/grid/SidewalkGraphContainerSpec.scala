...
@Ignore class SidewalkGraphContainerSpec extends FlatSpec with Matchers {
  ...
  "With in memory sidewalk graph container" should "filter unaccessible edges and ramps" in {
    ...
    val ramp1 = Ramp(Coordinate(10, 9), Some(1), "Callao 1234", isAccessible = true)
    val ramp2 = Ramp(Coordinate(10, 9), Some(2), "Callao 1234", isAccessible = true)
    val ramp3 = Ramp(Coordinate(10, 9), Some(3), "Callao 1234", isAccessible = true)
    val ramp4 = Ramp(Coordinate(10, 9), Some(4), "Callao 1234", isAccessible = true)
    ...
    //    val sidewalkContainer = InMemorySidewalkGraphContainer(List(sidewalk1, sidewalk2, sidewalk3, sidewalk4, sidewalk5), List(ramp1, ramp2, ramp3, ramp4))
    //
